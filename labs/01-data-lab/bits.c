 ...
 *   Rating: 4
 */
unsigned floatScale2(unsigned uf) {//1 8 23 bias=127
  unsigned a=uf&(1<<31);
  unsigned b=(uf>>23)&255;
  unsigned c=uf&((1<<23)-1);
  if(b==255)return uf;
  if(b==0)return a+(b<<23)+(c<<1);
  if(b==254)return a+(255<<23);
  return a+((b+1)<<23)+c;
}
/* 
